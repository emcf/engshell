...
def engshell_PREVIX(): return Style.RESET_ALL + os.getcwd() + ' ' + \
    Style.RESET_ALL + Fore.MAGENTA + "engshell" + Style.RESET_ALL + '>'
...
def clean_code_string(response_content):
    ...
    for code_languge in ['python', 'bash']:
        if response_content[:len(code_languge)] == code_languge:
            # remove python+newline blocks
            response_content = response_content[len(code_languge)+1:]
...
def containerize_code(code_string):
    ...
    code_string = code_string.replace(
        'your_unsplash_access_key_here', UNSPLASH_ACCESS_KEY)
    try:
...
def run_python(goal, debug=False, showcode=False):
    ...
    should_retry = should_debug or should_install or (
        (output is not None) and any([(err in output) for err in RETRY_ERRORS]))
    while should_retry:
        if should_install:
            ...
            prompt = returned_code + '\n\n The previous code gives the error ' + output + \
                ', given the following python code, rewrite the code with the error resolved:\n'
            print_status('debugging: ' + output)
            returned_code = LLM(prompt, mode='code')
            if showcode:
                print(returned_code,
                      end='' if returned_code[-1] == '\n' else '\n')
        success, output = containerize_code(returned_code)
        attempts += 1
        should_debug = debug and (
            attempts < MAX_DEBUG_ATTEMPTS) and (not success)
        should_retry = should_debug or any(
            [(err in output) for err in RETRY_ERRORS])
    if not success:
